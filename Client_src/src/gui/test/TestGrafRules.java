...
public class TestGrafRules  {
	
	ResourceBundle rb = ResourceBundle.getBundle("guitext", Locale.getDefault());
	private static final Logger log = Logger.getLogger(TestGrafRules.class.getName());
	...
	public TestGrafRules(String applicationTitle , String chartTitle,List<Testy> testy) {
	...
	private DefaultCategoryDataset createDataset(List<Testy> testy )
	   {
	      ...
	      for(Testy t : testy) {
	    	  dataset.addValue( t.getRules() , "Rules" , t.getExamdate().toLocaleString().substring(0,4) );
	      }
